...
void entry(struct initial_regs* regs) {
	...
	for (y = 0; y < display->height; y++) {
		for (x = 0; x < display->width; x++) {
			unsigned char* pixel = (unsigned char*)buffer + 4 * (y * display->pixelPerScanLine + x);
	...
	for (y = 0; y < (int)(display->vramSize / 4); y++) {
		((unsigned int*)display->vram)[y] = ((unsigned int*)buffer)[y];
	}
}
