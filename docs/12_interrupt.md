...
なお、このようなメモリにアクセスすることでI/Oを行う処理の場合、
特にI/O APICで用いられるあるメモリに書き込んだ値で別のメモリの役割を変えるような場合、
普通にC言語で書くと最適化によって挙動がおかしくなることがあるようである。
メモリマップドI/Oのポインタに`volatile`をつけることで、改善した。
